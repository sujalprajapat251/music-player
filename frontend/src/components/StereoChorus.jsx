...
function Knob2({ label = "Bite", min = -135, max = 135, defaultAngle, onValueChange, parameterIndex }) {
    const [angle, setAngle] = useState(defaultAngle ?? min);
    const [showTooltip, setShowTooltip] = useState(false);
    const knobRef = useRef(null);
    const dragging = useRef(false);
...
const StereoChorus = () => {
    ...
    const getCurrentParameterValue = (parameterIndex) => {
        ...
        switch (parameterIndex) {
            case 0: return 0; // Dist (Mix)
            case 1: return 45;   // Tone (Amount)
    ...
    return (
        <div className='bg-[#141414]'>
            <div className={`flex justify-between items-center w-[150px] h-[50px] sm:w-[190px] sm:h-[50px] md600:w-[220px] md:w-[230px] md:h-[55px] lg:w-[240px] xl:h-[60px] 2xl:w-[256px] 2xl:h-[64px] rounded-t-lg px-2 md:px-3 lg:px-8 xl:px-3 transition-colors duration-300 ${isPoweredOn ? 'bg-[#409C9F]' : 'bg-gray-600'
                ...
                />
                <p className='text-white text-[12px] md600:text-[16px]'>Stereo Chorus</p>
                <IoClose
            ...
            <div className={`w-[150px] h-[185px] sm:w-[190px] sm:h-[200px] md:w-[230px] md:h-[250px] lg:w-[240px] lg:h-[267px] xl:w-[240px] xl:h-[285px] 2xl:w-[256px] 2xl:h-[300px] bg-[#302f2f] relative ${!isPoweredOn ? 'opacity-50 pointer-events-none' : ''}`}>
                {/* Tone Knob - Top Right */}
                <div className="absolute top-[20px] left-[20px] sm:top-[25px] sm:left-[30px] md600:top-[25px] md600:left-[40px] lg:top-[55px] lg:left-[40px] xl:top-[45px] xl:left-[55px] xl:top-[35px] xl:left-[50px]">
                    <Knob1 label="Rate" min={-135} max={135} defaultAngle={getCurrentParameterValue(0)}
                        onValueChange={handleKnobChange}
                        parameterIndex={0} />
                </div>

                {/* Low cut Knob - Bottom Center */}
                <div className="absolute top-[35px] right-[35px] sm:top-[50px] sm:right-[60px] md600:top-[55px] md600:right-[50px] lg:top-[90px] lg:right-[55px] xl:top-[80px] xl:right-[45px] xl:top-[70px] xl:right-[50px] ">
                    <Knob1 label="Depth" min={-135} max={135} defaultAngle={getCurrentParameterValue(1)}
                        onValueChange={handleKnobChange}
                        parameterIndex={1} />
                </div>
                <div className="absolute bottom-[5px] right-[25px] sm:bottom-[10px] sm:right-[45px] md600:bottom-[20px] md600:right-[40px] md:bottom-[30px]  lg:bottom-[25px] lg:right-[50px] xl:bottom-[40px] xl:right-[40px] xl:bottom-[25px] xl:right-[40px] ">
                    <Knob2 label="Mix" min={-135} max={135} defaultAngle={getCurrentParameterValue(2)}
                        onValueChange={handleKnobChange}
                        parameterIndex={2} />
                </div>
            </div>
