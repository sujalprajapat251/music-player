...
class AudioEffectsController {
    ...
    startEffect(effectName, instanceId) {
        try {
            // Create effect instance using Tone.js
            ...
            switch (effectName) {
                ...
                case 'Overdrive':
                    ...
                    break;
                case 'Chorus':
                    effectInstance = new Tone.Chorus(params.defaultParams.frequency, params.defaultParams.rate, params.defaultParams.depth);
                    break;
                case 'Flanger':
                    effectInstance = new Tone.Chorus(params.defaultParams.frequency, params.defaultParams.rate, params.defaultParams.depth);
                    break;
                ...
                case 'Auto Pan':
                    ...
                    break;
                case 'Auto-Wah':
                    effectInstance = new Tone.AutoWah(params.defaultParams.frequency, params.defaultParams.Q, params.defaultParams.cutoff);
                    break;
                case 'Rotary':
                    effectInstance = new Tone.Tremolo(params.defaultParams.speed, params.defaultParams.depth);
                    break;
                case 'Stereo Chorus':
                    effectInstance = new Tone.Chorus(params.defaultParams.frequency, params.defaultParams.rate, params.defaultParams.depth);
                    break;
                case 'Tape Wobble':
                    effectInstance = new Tone.Vibrato(params.defaultParams.rate, params.defaultParams.depth);
                    break;
                case 'Clipper':
                    effectInstance = new Tone.Compressor(-params.defaultParams.threshold * 60, params.defaultParams.ratio);
                    break;
                case 'Crusher':
            ...
            if (effectInstance) {
                ...
                this.activeEffects.set(instanceId, { name: effectName, instance: effectInstance });
                console.log(`Started effect: ${effectName} with instance ID: ${instanceId}`);
            }
        } catch (error) {
    ...
    stopEffect(instanceId) {
        try {
            ...
            console.error(`Error stopping effect with instance ID ${instanceId}:`, error);
        }
    }
    ...
    updateEffectParameter(instanceId, paramName, value) {
        try {
            ...
            // Update parameter based on effect type and parameter name
            switch (paramName) {
                case 'distortion':
                case 'drive':
                case 'fuzz':
                ...
                case 'threshold':
                    if (effectInstance.threshold !== undefined) {
                        effectInstance.threshold.value = -value * 60; // Convert to dB
                    }
                    break;
                case 'ratio':
                ...
                case 'mix':
                    ...
                    break;
                default:
                    console.warn(`Unknown parameter: ${paramName} for effect instance: ${instanceId}`);
            }
            
            console.log(`Updated parameter ${paramName} to ${value} for effect instance: ${instanceId}`);
        } catch (error) {
            console.error(`Error updating parameter ${paramName} for instance ${instanceId}:`, error);
        }
    }
    ...
    playEffectDemo(effectName, duration = 4000) {
        ...
        // Use the existing audioEffectsPlayer for demo playback
        audioEffectsPlayer.playEffect(effectName).then(() => {
