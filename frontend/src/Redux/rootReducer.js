...
//             // Group actions that happen within a short time window (500ms)
//             const timeWindow = 500; // milliseconds
//             const lastAction = previousHistory.present && previousHistory.present.length > 0 
//                 ? previousHistory.present[previousHistory.present.length - 1] 
//                 : null;
            
//             if (lastAction && (now - lastAction.timestamp) < timeWindow) {
//                 // If the last action was of the same type and within the time window, group them
//                 if (lastAction.type === actionType) {
//                     return lastAction.groupId || `time-group-${Math.floor(now / timeWindow)}`;
//                 }
//             }
